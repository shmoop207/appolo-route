...
class Route {
    ...
    _addMiddleware(middleware, order = "tail", error = false) {
        ...
        if (utils_1.Objects.isPlain(middle) && (middle.order && middle.middleware)) {
            return this.middleware(middle.middleware, middle.order);
        }
        let id = engine_1.Util.getClassId(middle);
        if (id) {
            middleware = id;
        }
    ...
    middlewares(middlewares, order = "tail") {
        utils_1.Arrays.arrayify(middlewares).forEach(fn => this.middleware(fn, order));
        return this;
    }
    addHook(name, ...hook) {
