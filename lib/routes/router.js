...
class Router {
    constructor(_env, _injector, _agent) {
        ...
        this._onRouteAddedEvent = new events_1.Event();
        this._routes = [];
        this._hooks = new hooks_1.Hooks(_agent);
    }
    get hooks() {
        return this._hooks;
    }
    ...
    addRouteFromClass(fn) {
        ...
        Object.keys(routeData).forEach((key) => {
            ...
            utils_1.Reflector.getFnOwnMetadata(decorators_1.RouterDefinitionsCompiledSymbolController, fn, {})[key] = route;
            this.addRoute(route);
        });
    }
    _initRoute(route) {
        ...
        let middewares = helpers_1.Helpers.convertMiddleware(def.middleware, enums_1.MiddlewareType.MiddleWare).concat(helpers_1.Helpers.convertMiddleware(def.middlewareError, enums_1.MiddlewareType.Error));
    ...
    addMiddleware(path, middleware, error) {
        ...
        for (let i = 0; i < middleware.length; i++) {
            ...
            if (id) {
                middleware[i] = error ? (0, invokeMiddleWareError_1.invokeMiddleWareError)(id) : (0, invokeMiddleWare_1.invokeMiddleWare)(id);
            }
        }
        if (error) {
