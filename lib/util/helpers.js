...
class Helpers {
    ...
    static convertMiddleware(middleware, type) {
        ...
        for (let i = 0, len = middleware.length; i < len; i++) {
            ...
            if (id) {
                dto = type == IMiddleware_1.MiddlewareType.MiddleWare ? invokeMiddleWare_1.invokeMiddleWare(id) : type == IMiddleware_1.MiddlewareType.Error ? invokeMiddleWareError_1.invokeMiddleWareError(id) : invokeMiddleWareData_1.invokeMiddleWareData(id);
            }
            output.push(dto);
        }
        return output;
    ...
    static convertMiddlewareHooks(name, hooks) {
        return Helpers.convertMiddleware(hooks, name == agent_1.HooksTypes.OnSend ? IMiddleware_1.MiddlewareType.Data : name == agent_1.HooksTypes.OnError ? IMiddleware_1.MiddlewareType.Error : IMiddleware_1.MiddlewareType.MiddleWare);
    }
}
